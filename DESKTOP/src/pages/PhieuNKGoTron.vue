    ...
    <div v-if="phieu" class="print-area">
      ...
      <div class="bktm-page">
        <div class="bktm">
          ...
          <div class="bktm-date-row q-mt-md">
            <div class="date-col-empty"></div>
            <div class="date-col">Ngày {{ ngay }} tháng {{ thang }} năm {{ nam }}</div>
          </div>
          ...
          <div class="sign-area">
            <div class="sign-col">
              ...
              <div class="sign-space"></div>
              <div class="sign-name">{{ NGUOI_LAP_BK }}</div>
            </div>
            <div class="sign-col">
...
export default {
  ...
  methods: {
    ...
    async loadCodes() {
      ...
      // Suy ra fromDate/toDate = đầu tháng → cuối tháng (giữ nguyên API getCodebangiaogotron)
    ...
    applyXuongConfig() {
      ...
      this.NGUOI_LAP_BK = cfg.nguoi_lap_bk || "";
    ...
    buildSecBKTM(ws, p, cfg, startRow) {
      ...
      this.setCell(ws, `F${cur}`, "(Ký tên, đóng dấu)",
        { merge: `J${cur}`, italic: true, size: 9, center: true });
      cur += 4;
      // Người lập bảng kê: lấy từ XUONG_XE.nguoi_lap_bk
      this.setCell(ws, `A${cur}`, cfg.NGUOI_LAP_BK || "", { merge: `E${cur}`, bold: true, center: true });
    ...
    wordSecBKTM(p, cfg) {
      ...
      return `
        ...
        <table class="sign-2col" style="margin-top:4pt;"><tr>
          <td></td>
          <td><p class="italic" style="text-align:center;">${e(dateBottom)}</p></td>
        </tr></table>
        <table class="sign-2col"><tr>
          <td>
            <p class="bold italic">Người lập bảng kê</p>
            <p class="small italic">(Ký, ghi rõ họ tên)</p>
            <p class="sign-name">${e(cfg.NGUOI_LAP_BK || "")}</p>
          </td>
          <td>
...
.bktm-date-row .date-col-empty { width: 45%; }
.bktm-date-row .date-col {
  width: 45%;
  text-align: center;
  font-style: italic;
  font-size: 12px;
